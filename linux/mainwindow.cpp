    ...
    ui(new Ui::MainWindow)
{
    ...
    //writer = boost::shared_ptr<cv::VideoWriter>(new cv::VideoWriter("Video.avi", VideoWriter::fourcc('X', 'V', 'I', 'D'), 30, Size(600,600))) ;
...
void MainWindow::initNet()
{
    ...
    //---------------导入视频数据------------------
    string val_path = "/home/zyl/qtWorkPath/AutoControlSystem/20200928172405.mp4";
...
{
    if(detect != nullptr)
        SSD_Detect::addIndex += 1;
        SSD_Detect::addIndex %= 4;
        ui->label_10->setText(QString("Index: %1").arg(SSD_Detect::index));
}
