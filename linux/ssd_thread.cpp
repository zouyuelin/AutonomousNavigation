...
void inline SSD_Detect::getConfidenceCentre(vector< vector<Point2f> > tens,Mat &frame)
{
    ...
    //------------------------------------------------------------get the index of the select point-----------------------------------------------start

    if(hasClosePoint == 0 && theTempCentre.size() !=0)
    {
            selectPoint = theTempCentre[0];
            hasClosePoint = 1;
    }
    for(int i=0;i<theTempCentre.size();i++)
    {
        if(distance_(theTempCentre[i],selectPoint)<40)
        {
            index = i;
            selectPoint = theTempCentre[i];
            break;
        }
        if(i == theTempCentre.size()-1)
        {
            hasClosePoint = 0;
        }
    }
    if(theTempCentre.size()!=0)
    {
            index = (index+addIndex)%theTempCentre.size();
            selectPoint = theTempCentre[index];
    }
    //------------------------------------------------------------get the index of the select point-----------------------------------------------end
