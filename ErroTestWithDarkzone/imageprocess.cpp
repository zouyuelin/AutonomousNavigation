...
imgProcess::imgProcess(std::string videoPath,QObject *parent):cameraMatrix(cv::Mat::zeros(3,3,CV_64F)),
    distCoeffs(cv::Mat::zeros(1,5,CV_64F)),StartProcess(true),offset(0),messageSend(cv::Mat::zeros(1,3,CV_64F))
{
...
cv::Mat imgProcess::PreProcess(cv::Mat frame,cv::Mat &raw)
{
    ...
    //大津算法
    int thred = OtsuAlgThreshold(frameRepat);
    threshold(frameRepat,
              frameRepat,
              thred,
              255,
              cv::THRESH_BINARY_INV);//cv::THRESH_BINARY_INV cv::THRESH_OTSU
